...
def test_add_task_invalid_jadwal(client):
    with patch('app.get_db') as mock_db:
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_db.return_value = mock_conn
        ...
        response = client.post('/add', data={
            'task': 'Test Task',
            'jadwal': 'jam9pagi'  # format salah
        })
        assert response.status_code == 200  # balik ke halaman, bukan redirect
        assert 'tidak valid' in response.data.decode()
...
def test_add_task_empty(client):
    with patch('app.get_db') as mock_db:
        ...
        assert response.status_code == 200
        assert 'kosong' in response.data.decode()